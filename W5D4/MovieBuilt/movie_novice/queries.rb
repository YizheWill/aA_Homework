...
def below_average_years
  ...
  Movie.select("count(title) as bad_movies, yr").where("score <?", 5).group("yr").order("yr DESC")
end
...
def pulp_fiction_actors
  ...
  Actor.joins(:movies).where("movies.title = ?", "Pulp Fiction").select("actors.id, actors.name")
end
...
def uma_movies
  ...
  Movie.select("movies.id, movies.title, movies.yr").joins(:actors).where("actors.name = ?", "Uma Thurman").order("yr desc")
end
